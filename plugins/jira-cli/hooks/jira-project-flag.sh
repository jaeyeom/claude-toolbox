...
#
# Blocked pattern:
#   - jira issue create without -p or --project
#
# Exit codes:
#   0 = allow the command
...
if echo "$COMMAND" | grep -qE '\bjira\b[^|;]*\bissue\b[^|;]*\bcreate\b'; then
	if ! echo "$COMMAND" | grep -qE '\bjira\b[^|;]*\bissue\b[^|;]*\bcreate\b[^|;]*(\s-p\s|\s-p[^-\s]|--project\b)'; then
		deny "jira issue create must include -p/--project flag. Relying on the default project is error-prone (see ankitpokhrel/jira-cli#979)."
	fi
fi
