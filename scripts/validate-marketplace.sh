...
# ---------------------------------------------------------------------------
# 11. Ban --plugin flag in install commands
# ---------------------------------------------------------------------------
echo "--- Checking for banned --plugin flag in install commands ---"

banned_count=0
while IFS= read -r -d '' file; do
	if grep -n -- '--plugin' "$file" >/dev/null 2>&1; then
		while IFS=: read -r lineno line; do
			error "$file:$lineno: uses banned '--plugin' flag: $line"
			banned_count=$((banned_count + 1))
		done < <(grep -n -- '--plugin' "$file")
	fi
